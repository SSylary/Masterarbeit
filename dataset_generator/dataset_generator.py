            ...
            for row in data:
                if self.label == row[1]:
                    ...
                    self.possible_crops.append({'filename': filename, 'polygonCoordinates': polygonCoorList,
                        'bboxCenter': self.getBboxCenter(bbox), 'bbox': bbox, 'crop': crop,'brightnessFactor':self.brightnessFactor})

    def getCrop(self, filename, bbox, polygon):
        im = Image.open(os.path.join(
            '/disk/users/fn875/no_backup/xzc/pipenv_project/Mask_RCNN/AL_test/dataset_generator/labeled_images', filename)).convert('RGBA')
...
class CsvFile():
    ...
    def parseDict(self, cropsToPaste, imFilename):
        ...
        for crop in cropsToPaste:
            crops.append([imFilename, crop['label'], crop['newPolygon'], crop['brightnessFactor']])
...
class NewImage:
    ...
    csvFile = CsvFile('/disk/users/fn875/no_backup/xzc/pipenv_project/Mask_RCNN/AL_test/labels/test_data_labels')  #

    def __init__(self, choosenBlocks):
        # -----TODO----- background need change
        fileList = ['bg1.jpg', 'bg2.png', 'bg3.jpg','bg4.png','bg5.png','bg6.png','bg7.png']
        # self.background = Image.new('RGB', (IMAGE_WIDTH, IMAGE_HEIGHT), (0, 0, 0))
        bgId = fileList[random.choice([0,1,2,3,4,5,6])]
        im = Image.open(os.path.join('/disk/users/fn875/no_backup/xzc/pipenv_project/Mask_RCNN/AL_test/dataset_generator/background', bgId)).convert('RGBA')
        brightnessFactor = random.choice([0.6,0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.4])
        im = ImageEnhance.Brightness(im).enhance(brightnessFactor)
        ...
        for block in choosenBlocks:
            self.crops.append(
                {'label': block.label, 'im': block.choosenCrop['crop'],
                 ...
                 'bboxCenter': block.choosenCrop['bboxCenter'], 'bbox': block.choosenCrop['bbox'],'brightnessFactor':block.choosenCrop['brightnessFactor']})
    ...
    def adaptCrop(self):
        ...
        print('first crop')
        tmpCrop = {}
        tmpCrop['label'], tmpCrop['brightnessFactor'],[tmpCrop['im'], tmpCrop['angle']] = self.crops[0]['label'],self.crops[0]['brightnessFactor'], self.randomRotate(self.crops[0])
        ...
        if len(self.crops) != 1:
            for crop in self.crops[1:]:
                ...
                while flag:
                    ...
                    tmpCrop = {}
                    tmpCrop['label'],tmpCrop['brightnessFactor'], [tmpCrop['im'], tmpCrop['angle']] = crop['label'],crop['brightnessFactor'],  self.randomRotate(crop)
    ...
    def pasteCrops(self):
        for crop in self.cropsToPaste:
            ...
            brightnessFactor = random.choice([0.6,0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.4])
            crop['im'] = ImageEnhance.Brightness(crop['im']).enhance(brightnessFactor)
            crop['brightnessFactor'] = brightnessFactor
            self.background.paste(crop['im'], mask=crop['im'].split()[3], box=(posX, posY)) # crop['newBbox'][0]
        #print("*****@@@@*****")
        #print(np.array(self.background).shape)
        self.background = self.background.convert('RGB')
        #print(np.array(self.background).shape)
        self.saveImage()
    ...
    def saveImage(self):
        ...
        # change new folder name here------>
        self.background.save(os.path.join('/disk/users/fn875/no_backup/xzc/pipenv_project/Mask_RCNN/AL_test/test_data', self.filename + '.png'))
...
if __name__ == '__main__':
    ...
    IMAGE_HEIGHT = 2054  # pixels
    IMAGE_WIDTH = 2456
    polygon_filename = os.path.abspath('/disk/users/fn875/no_backup/xzc/pipenv_project/Mask_RCNN/AL_test/dataset_generator/Polygones_labels.csv')
